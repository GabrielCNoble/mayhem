...
__declspec(dllexport) void r_DrawLine(vec3_t a, vec3_t b, vec3_t color)
{
    ...
    r_QueueCmd(R_CMD_DRAW_LINE, &data, sizeof(data));
}
...
__declspec(dllexport) void r_DrawTriangle(vec3_t a, vec3_t b, vec3_t c, vec3_t color, int wireframe, int backface_culling, int depth_test)
{
    ...
    r_QueueCmd(R_CMD_DRAW_TRIANGLE, &data, sizeof(data));
}
...
__declspec(dllexport) void r_DrawCapsule(vec3_t position, float height, float radius)
{
    ...
    r_QueueCmd(R_CMD_DRAW_CAPSULE, &data, sizeof(data));
}
...
__declspec(dllexport) void r_DrawPoint(vec3_t position, vec3_t color, float size, int smooth)
{
    ...
    r_QueueCmd(R_CMD_DRAW_POINT, &data, sizeof(data));
}
...
__declspec(dllexport) void r_DrawDebug(struct view_t *view)
{
    struct draw_debug_data_t data;

    data.view_projection_matrix = view->view_projection_matrix;

    r_QueueCmd(R_CMD_DRAW_DBG, &data, sizeof(data));
}
